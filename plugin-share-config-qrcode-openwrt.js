...
const share = async (profile) => {
  ...
  if (profile.tunConfig) {
    ...
    profile.tunConfig['auto-route'] = false
    profile.tunConfig['auto-detect-interface'] = false
    profile.tunConfig['dns-hijack'] = profile.tunConfig['dns-hijack']?.length ? profile.tunConfig['dns-hijack'] : ['any:53']
    profile.tunConfig['strict-route'] = false
  }
  ...
  try {
    await Plugins.alert(
      ...
      '### 注意事项：\n\n' +
        ...
        port +
        '\n' +
        ' - 扫描二维码后，若 OpenWrt 未自动导入，请复制链接手动添加\n' +
        ' - 如果仍无法导入，请更换不同二维码尝试\n\n' +
        '|分享链接|二维码|\n|-|-|\n' +
